  ...
  if (items.length === 0) {
    return (
      <div className="max-w-6xl mx-auto px-5 py-24 text-center">
        ...
        <Link
          to="/shop"
          className="inline-flex items-center gap-2 mt-6 rounded-full text-white font-semibold px-6 py-3.5 transition-all hover:shadow-lg hover:-translate-y-0.5"
    ...
    <div className="max-w-6xl mx-auto px-5 py-14">
      ...
      <div className="grid md:grid-cols-[1.5fr_1fr] gap-10 mt-10">
        <div className="divide-y divide-slate-100">
          {items.map((i, idx) => (
            <Reveal key={i.productId} delay={idx * 60}>
              <div className="py-5 flex items-start gap-4 border-b border-slate-100 hover:bg-slate-50/60 rounded-lg transition-colors -mx-2 px-2">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <button onClick={() => updateQty(i.productId, i.qty - 1)} className="h-7 w-7 rounded-full border border-slate-200 flex items-center justify-center hover:border-[#3E4095] hover:text-[#3E4095] transition-colors">
                      <Minus size={12} />
                    </button>
                ...
                <div className="text-right">
                  ...
                  <button onClick={() => removeItem(i.productId)} className="mt-3 text-slate-300 hover:text-red-500 transition-colors">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
        ...
        <Reveal delay={150}>
          <div className="rounded-2xl border border-slate-200 p-6 h-fit" style={{ background: 'linear-gradient(160deg, #F7F7FC, white)' }}>
            ...
            <button
              onClick={(e) => { onRippleClick(e); navigate('/checkout') }}
              className="relative overflow-hidden mt-6 w-full rounded-full text-white font-semibold py-3.5 transition-all hover:shadow-lg hover:-translate-y-0.5"
              style={{ background: 'linear-gradient(135deg, #3E4095, #33356E)' }}
            >
              Proceed to checkout
